...
def compute_global_limits(
    ...
    c_all = np.concatenate(
        [np.asarray(snaps[k]["c_plus_molm3"]) for k in profile_labels]
        + [np.asarray(snaps[k]["c_minus_molm3"]) for k in profile_labels]
    )
    yconc = nice_limits(float(np.min(c_all)), float(np.max(c_all)), nticks=5)

    phi_all_mV = np.concatenate([1e3 * np.asarray(snaps[k]["phi_V"]) for k in profile_labels])
...
def plot_profile_panel(
    ...
    ax.plot(z_nm, c_plus-150, color=COLOR_POS, linestyle="-", label=r"$c_+(z)$",linewidth=0.8)
    ax.plot(z_nm, c_minus-150, color=COLOR_NEG, linestyle="-", label=r"$c_-(z)$",linewidth=0.8)
    ...
    #ax2.set_ylabel(r"Electric potential (mV)")
    ...
    ax.set_xlim(600, 900)
    ax.set_xticks([600,750,900])
    #ax.set_ylim(limits["yconc"][0], limits["yconc"][1])
    ax.set_ylim(-100, 200)
    ax.set_yticks([-100,0,200])
    #ax2.set_ylim(limits["yphi"][0], limits["yphi"][1])
    ax2.set_ylim(-250, 150)
    ax2.set_yticks([-250,0,150])
